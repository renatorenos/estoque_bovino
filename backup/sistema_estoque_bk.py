...
class ControladorEstoque:
        ...
        for tipo, data, quantidade, codigo in self.movimentacoes_ordenadas:
            if tipo == 'E':
                ...
                if not produto.registrar_saida(data, quantidade):
                    print(f"ALERTA: Tentativa de venda sem estoque suficiente em {data.strftime('%Y-%m-%d')}")
                    print(f"Produto: {produto.codigo} - {produto.descricao}")
                    print(f"Quantidade solicitada: {quantidade:.2f}")
                    print(f"Saldo disponível: {produto.saldo_atual:.2f}")
                    print()
    ...
    def gerar_relatorio(self):
        ...
        print(f"Quantidade Total Entrada: {self.entrada_total:.2f} kg")
        ...
        if abs(total_percentual - 100) > 0.01:
            print(f"ALERTA: Soma dos percentuais ({total_percentual:.2f}%) não totaliza 100%")
        ...
        if produtos_negativos:
            ...
            for produto in produtos_negativos:
                print(f"- {produto.codigo} {produto.descricao}: {produto.saldo_atual:.2f} kg")
